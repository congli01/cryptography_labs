...
void mixColumns(int array[4][4])
{
    ...
    for(i = 0;i < 4;i++)
    {
        for(j = 0;j < 4;j++)
        {
            array[i][j] = GFMul(colM[i][0],cp[0][j]) ^ GFMul(colM[i][1],cp[1][j]) ^ GFMul(colM[i][2],cp[2][j]) ^ GFMul(colM[i][3],cp[3][j]);
        }
    }
}
...
void deMixColumns(int array[4][4])
{
    ...
    for(i = 0;i < 4;i++)
    {
        for(j = 0;j < 4;j++)
        {
            array[i][j] = GFMul(deColM[i][0],cp[0][j]) ^ GFMul(deColM[i][1],cp[1][j]) ^ GFMul(deColM[i][2],cp[2][j]) ^ GFMul(deColM[i][3],cp[3][j]);
        }
    }
}
...
void addRoundKey(int array[4][4], int round)
{
    //请补充代码
    int i, j;
    int k_inv[4][4]; // 密钥矩阵的转置
    for (i = 0;i < 4;i++)
    {
        splitIntToArray(w[round*4+i], k_inv[i]);
    }
    for(i = 0;i < 4;i++)
    {
        for(j = 0;j < 4;j++)
        {
            array[i][j] = array[i][j] ^ k_inv[j][i];
        }
    }
}
...
void extendKey(char *key)
{
   ...
   for(i = 0;i < Nk;i++)
   {
       ...
       w[i] = getWordFromStr(str);
   }
   for(i = Nk;i < Nb*(Nr+1);i++)
   {
       if(i%Nk == 0)
       {
           splitIntToArray(w[i-1], temp_array);
           // 循环左移一个字节
           leftLoop4int(temp_array, 1);
           // 字节代换
           for(int j=0;j < 4;j++)
           {
               temp_array[j] = getNumFromSBox(temp_array[j]);
           }
           temp = mergeArrayToInt(temp_array) ^ Rcon[i/Nk - 1];
           w[i] = w[i-Nk] ^ temp;
       }
       else
       {
